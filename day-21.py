# fused_chaos_with_quantum_and_dirac_influence.py (Day 21 - Let It Glow!)
# MicroPython on RP2040 (Pico) - Main Controller
#
# Day 21: Addressable LED String Lights (~15 LEDs)
# Day 20: DHT20 I2C Temperature & Humidity Sensor
...
# - Slave Pico handles GPIO expansion and sensors
...
# -------------------------
# DHT20 on slave Pico I2C bus (GP14=SDA, GP15=SCL)
# Request readings via UART

def read_temperature_humidity():
    """Read temperature and humidity from DHT20 sensor on slave Pico
    Protocol: 'T' command requests sensor reading
    Returns: (temperature_C, humidity_percent) or (None, None) on error
    """
    try:
        # Send temperature/humidity request
        uart_expander.write(b'T')

        # Wait for response (7 bytes: temp_int, temp_dec, hum_int, hum_dec, status, checksum1, checksum2)
        # Or simplified: 4 bytes (temp_int, temp_frac, hum_int, hum_frac)
        start = time.ticks_ms()
        while uart_expander.any() < 4:
            if time.ticks_diff(time.ticks_ms(), start) > 200:  # 200ms timeout
                return (None, None)
            time.sleep_ms(5)

        # Read 4 bytes: temp_int, temp_frac, hum_int, hum_frac
        data = uart_expander.read(4)
        if len(data) == 4:
            temp_c = data[0] + (data[1] / 100.0)  # e.g., 23 + 0.45 = 23.45°C
            humidity = data[2] + (data[3] / 100.0)  # e.g., 55 + 0.32 = 55.32%
...
# -------------------------
# Day 21: LED String Lights Control (Let It Glow)
# -------------------------
# ~15 LED addressable string lights on slave Pico

def send_string_pattern(pattern_mode, speed, brightness):
    """Send LED string pattern command to slave Pico
    Protocol: 'L' + pattern_mode (0-255) + speed (0-255) + brightness (0-255)

    Pattern modes:
    0: Chase (running lights)
    1: Twinkle (random sparkles)
    2: Wave (smooth pulsing)
    3: Chaos (driven by entropy)
    """
    try:
        # Convert to bytes
        mode_byte = int(pattern_mode) & 0xFF
        speed_byte = int(speed * 255) & 0xFF
        brightness_byte = int(brightness * 255) & 0xFF

        # Send string command: 'L' followed by 3 bytes
        uart_expander.write(b'L')
        uart_expander.write(bytes([mode_byte, speed_byte, brightness_byte]))
...
def update_string_lights(chaos_val, spinor, humidity):
    """Update LED string lights with chaos-driven patterns
    Pattern selection and speed driven by chaos system
    """
    # Pattern mode cycles through different effects based on chaos
    pattern_mode = int(chaos_val * 4) % 4  # 0-3

    # Speed increases with spinor energy
    speed = 0.3 + abs(spinor[0]) * 0.4 + abs(spinor[1]) * 0.3
    speed = min(1.0, speed)

    # Brightness modulated by humidity (if available)
    if humidity is not None:
        brightness = 0.5 + (humidity / 100.0) * 0.3  # 50-80% brightness range
    else:
        brightness = 0.6
    ...
    # Send pattern to slave Pico
    send_string_pattern(pattern_mode, speed, brightness)
...
while True:
    ...
    # -------------------------
    # Day 20: Read DHT20 Temperature & Humidity Sensor
    # -------------------------
    temp_c, humidity = read_temperature_humidity()

    if temp_c is not None and humidity is not None:
        print("DHT20 - Temp: {:.1f}°C, Humidity: {:.1f}%".format(temp_c, humidity))

        # Use temperature to modulate chaos parameter (20-30°C nominal)
        # Cooler = slower chaos, Warmer = faster chaos
        temp_influence_dht = ((temp_c - 20) / 10.0) * 0.1  # -0.1 to +0.1 range

        # Use humidity to affect display brightness/colors (40-60% nominal)
        # Lower humidity = dimmer, Higher humidity = brighter
        humidity_influence = (humidity - 50) / 50.0  # -1.0 to +1.0 range
    else:
        print("DHT20 - Reading failed")
        temp_influence_dht = 0.0
        humidity_influence = 0.0
    ...
    # Logistic map chaos
    # -------------------------
    # Adjust r parameter based on spinor, button, temp, pot, tilt, BEAM, DIP, and DHT20
    r = 3.5 + s0 * 0.49 + button_pressure * 0.1 + temp_influence * 0.15 + pot_norm * 0.2 + tilt_influence * 0.15 + beam_influence * 0.18 + dip_switch_value * 0.12 + temp_influence_dht * 0.1
    x = r * x * (1 - x)
    ...
    # Modulate with humidity (higher humidity = more segments lit)
    bar_level = x + (humidity_influence * 0.15)
    bar_level = max(0.0, min(1.0, bar_level))  # Clamp to 0-1
    update_bar_graph(bar_level)
    ...
    # -------------------------
    # Day 21: Update LED String Lights
    # -------------------------
    # Chase/twinkle patterns driven by chaos and spinor
    update_string_lights(x, spin, humidity)
